...
export function deploymentIdPour(env: EnvironnementDeploiement): string | undefined {
  for (const cle of ["VERCEL_DEPLOYMENT_ID", "VERCEL_GIT_COMMIT_SHA", "NEXT_DEPLOYMENT_ID"] as const) {
    ...
    // Preview and production can build the same commit. Vercel's per-deployment identifier
    // keeps those builds distinct; the SHA remains a fallback outside that environment.
