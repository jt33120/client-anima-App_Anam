-- Les registres introduits par 0083 et 0084 portent des données rattachées à une personne : une
-- admission mensuelle au quota, et le jour où sa conversation avec Anam a commencé. Ils doivent
-- donc suivre le droit d’accès (art. 15), même s’ils restent deny-by-default dans l’application.
--
-- Ce corps repart intégralement de la dernière définition, 0079. Les seules capacités ajoutées aux
-- retraits sont la clé d'idempotence du quota et le jeton du bail d'ouverture. L'échéance du bail,
-- elle, reste exportée : c'est une donnée de traitement lisible, pas un secret permettant d'agir.
