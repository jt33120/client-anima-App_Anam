...
describe("[LE CŒUR] ce que le portail montre", () => {
  ...
  it("montre le profil officiel d’Anam de dos, chargé en priorité, et aucun arbre", () => {
    ...
    const portrait = container.querySelector('img[src="/scene/veille/anam-veille.png"]');
    expect(portrait, "le profil de dos d’Anam manque au portail").not.toBeNull();
    expect(portrait!.getAttribute("loading")).toBe("eager");
    expect(container.querySelector("canvas"), "un arbre subsiste dans le portail").toBeNull();
  });
});
