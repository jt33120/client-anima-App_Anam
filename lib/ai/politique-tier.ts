...
export function tierPour(capacite: CapaciteIa, niveauSecurite: NiveauSecurite = 0): TierIa {
  ...
  // ══════════════════════════════════════════════════════════════════════════════════════════════
  // LE TEXTE DU JOUR EST PASSÉ AU FORT LE 2026-09-07, ET LES TROIS ARGUMENTS D'HIER SONT TOMBÉS.
  // ══════════════════════════════════════════════════════════════════════════════════════════════
  //
  // Cette ligne rendait « leger », et le raisonnement écrit ici disait : « sa charge utile ne
  // contient aucun mot d'elle », « il n'y a rien à comprendre de travers sur quelqu'un », « le fort
  // n'ajouterait ici que du coût ». Les trois étaient vrais. Aucun ne l'est plus :
  //
  //   • la charge utile porte maintenant son prénom, ses signes natals, le nom des branches de son
  //     arbre et ce qu'Anam a retenu d'elle ;
  //   • il y a donc quelque chose à comprendre de travers sur quelqu'un — c'est le prédicat exact
  //     qui met `lecture` au fort, deux blocs plus haut ;
  //   • et la sortie est PERSISTÉE pour quarante-huit heures puis re-servie telle quelle, ce qui est
  //     le prédicat qui met `compactage` au fort.
  //
  // ⚠️ ÉCRIT EXPLICITEMENT, ET NON OBTENU EN SUPPRIMANT LA LIGNE. Retirer ce `if` donnerait « fort »
  // par le repli final, et le dépôt refuse le tier hérité : un fort par héritage se lit comme un
  // fort par décision, et personne ne saurait plus lequel des deux il relit.
  //
